...
export default function DietTracker() {
  ...
  const [usedSwaps,   setUsedSwaps]   = useState([]);  // ["Monday_dinner", ...] pairs that have been logged
  const [weights,     setWeights]     = useState([]);
  const [view,        setView]        = useState("plan"); // plan | progress | weight
  const [modal,       setModal]       = useState(null);   // swap modal
  const [resetModal,  setResetModal]  = useState(false);
  const [weightModal, setWeightModal] = useState(false);
  const [weightInput, setWeightInput] = useState("");
  const [justChecked, setJustChecked] = useState(null);
  const [confirmReset,setConfirmReset]= useState(null); // "day" | "week"
  ...
  // ── Get effective meal text ──
  const getEffective = (day, meal) => {
    const swapKey = swaps[`${day}_${meal.id}`]; // "Wednesday_dinner"
    if (!swapKey) return { text: meal.text, isSwapped: false };
    const [srcDay, srcMealId] = swapKey.split("_");
    const srcMeal = DIET_PLAN[srcDay]?.meals.find(m => m.id === srcMealId);
    return srcMeal
      ? { text: srcMeal.text, isSwapped: true, fromDay: srcDay, swapKey }
  ...
  const toggleMeal = (day, mealId) => {
    ...
    const newLogs   = { ...logs, [day]: updated };
    setLogs(newLogs);
    saveLog(day, updated);
    if (!wasLogged) {
      // mark this swap as used so it can't be swapped onto another day
      const swapKey = swaps[`${day}_${mealId}`];
      if (swapKey && !usedSwaps.includes(swapKey)) {
        const u = [...usedSwaps, swapKey];
  ...
  // ── Apply swap ──
  const applySwap = (targetDay, targetMealId, srcDay, srcMealId) => {
    const key     = `${targetDay}_${targetMealId}`;
    const swapVal = `${srcDay}_${srcMealId}`;
    const updated = { ...swaps, [key]: swapVal };
  ...
  const resetDay = (day) => {
    ...
    // remove swaps for this day
    const s = { ...swaps };
    Object.keys(s).forEach(k => { if (k.startsWith(`${day}_`)) delete s[k]; });
  ...
  const getDayProg = (day) => {
    ...
    const done  = DIET_PLAN[day].meals.filter(m => logs[day]?.[m.id]).length;
    return { done, total, pct: total ? Math.round((done/total)*100) : 0 };
  };
  const getWeekProg = () => {
    let t=0,d=0; DAYS.forEach(day => { const p=getDayProg(day); t+=p.total; d+=p.done; });
  ...
  const lostWt   = weights.length >= 2 ? (startWt - latestWt).toFixed(1) : "—";

  // ── Swap Modal ──
  const SwapModal = () => {
    if (!modal) return null;
    const { day, mealId, label } = modal;
    const currentSwap = swaps[`${day}_${mealId}`];
    const isLogged    = logs[day]?.[mealId];
    // Other days that have the same mealId
    ...
    return (
      <div onClick={() => setModal(null)} style={{ position:"fixed",inset:0,background:"rgba(44,24,16,0.6)",zIndex:999,display:"flex",alignItems:"flex-end",justifyContent:"center" }}>
        <div onClick={e=>e.stopPropagation()} style={{ background:"#FDF6EC",borderRadius:"20px 20px 0 0",width:"100%",maxWidth:430,maxHeight:"80vh",display:"flex",flexDirection:"column",boxShadow:"0 -8px 40px rgba(44,24,16,0.2)" }}>
          <div style={{ padding:"14px 20px 12px",borderBottom:"1px solid #EDE0D4",flexShrink:0 }}>
            <div style={{ width:36,height:4,background:"#D4C4B8",borderRadius:99,margin:"0 auto 14px" }}/>
            <div style={{ display:"flex",alignItems:"center",justifyContent:"space-between" }}>
              <div>
                <p style={{ fontSize:11,color:"#A08070",textTransform:"uppercase",letterSpacing:1,marginBottom:3 }}>🔄 Swap · {selectedDay}'s {label}</p>
                <h3 style={{ fontFamily:"'Playfair Display',serif",fontSize:18,color:"#2C1810" }}>Pick from another day</h3>
              </div>
            ...
            {isLogged && (
              <div style={{ background:"#FFF0EE",border:"1px solid #F5C4BC",borderRadius:10,padding:"8px 12px",marginTop:10 }}>
          ...
          <div style={{ overflowY:"auto",padding:"12px 16px 28px",flex:1 }}>
            ...
            {otherDays.length === 0 && (
              <div style={{ textAlign:"center",padding:"24px 0",color:"#A08070",fontSize:13 }}>
                ...
                No other days have a {label} meal to swap with.
              </div>
            )}
            ...
            {otherDays.map(srcDay => {
              ...
              const isCurrent  = currentSwap === swapVal;
              // Is this swap already logged on some other day's slot?
              const isUsed     = usedSwaps.includes(swapVal) && !isCurrent;
              const srcPlan    = DIET_PLAN[srcDay];

              return (
                <div key={srcDay} onClick={()=>{ if(!isUsed && !isLogged) applySwap(day,mealId,srcDay,mealId); }} style={{
                  border:     isCurrent ? "2px solid #5A8A6A" : isUsed ? "1.5px dashed #D4C4B8" : "1.5px solid #EDE0D4",
                  background: isCurrent ? "#F0F8F2"           : isUsed ? "#F8F4F0"               : "#fff",
                  borderRadius:14,padding:"12px 14px",marginBottom:10,
                  cursor: isUsed||isLogged ? "not-allowed" : "pointer",
          ...
          {confirmReset ? (
            <div style={{ background:"#FFF0EE",border:"1px solid #F5C4BC",borderRadius:14,padding:16,marginBottom:12 }}>
              ...
              <p style={{ fontSize:12,color:"#A08070",marginBottom:14 }}>
                {confirmReset==="day"  ? "All logs and swaps for this day will be cleared."
                 :confirmReset==="week"? "All logs and swaps for this week will be cleared."
                 :"A fresh week will start. Previous week data stays saved."}
              </p>
      ...
      <div onClick={()=>setWeightModal(false)} style={{ position:"fixed",inset:0,background:"rgba(44,24,16,0.6)",zIndex:999,display:"flex",alignItems:"flex-end",justifyContent:"center" }}>
        <div onClick={e=>e.stopPropagation()} style={{ background:"#FDF6EC",borderRadius:"20px 20px 0 0",width:"100%",maxWidth:430,padding:"20px 20px 40px",boxShadow:"0 -8px 40px rgba(44,24,16,0.2)" }}>
          ...
          <div style={{ display:"flex",gap:10,marginBottom:16,alignItems:"center" }}>
            <div style={{ flex:1,background:"#fff",border:"2px solid #C4756A",borderRadius:14,padding:"14px 16px",display:"flex",alignItems:"center",gap:8 }}>
          ...
          <div style={{ display:"flex",gap:8,marginBottom:16,flexWrap:"wrap" }}>
            {[75,76,77,78,79,80].map(v=>(
              <button key={v} onClick={()=>setWeightInput(String(v))} style={{ border:`1.5px solid ${weightInput==v?"#C4756A":"#EDE0D4"}`,background:weightInput==v?"#FFF0EE":"#fff",borderRadius:20,padding:"6px 14px",fontSize:13,fontWeight:600,color:weightInput==v?"#C4756A":"#A08070",cursor:"pointer" }}>{v}</button>
            ))}
          </div>
          <button onClick={addWeight} style={{ width:"100%",padding:"14px",border:"none",borderRadius:14,background:"linear-gradient(135deg,#C4756A,#D4896E)",fontSize:15,fontWeight:700,color:"#fff",cursor:"pointer" }}>
            Save Weight
          </button>
        </div>
      </div>
    ...
    <div style={{ fontFamily:"'DM Sans',sans-serif",background:"#FDF6EC",minHeight:"100vh",maxWidth:430,margin:"0 auto",paddingBottom:80 }}>
      <style>{`
        ...
        *{box-sizing:border-box;margin:0;padding:0;}
        .mc{transition:all 0.2s ease;} .mc:active{transform:scale(0.98);}
        ...
        .db{transition:all 0.15s ease;} .db:active{transform:scale(0.93);}
        ...
        <div style={{ position:"relative" }}>
          <div style={{ display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:4 }}>
            ...
            <div style={{ display:"flex",gap:8 }}>
              <button onClick={()=>setWeightModal(true)} style={{ border:"none",background:"rgba(255,255,255,0.2)",borderRadius:20,padding:"5px 12px",fontSize:11,fontWeight:600,color:"#fff",cursor:"pointer" }}>⚖️ Log Weight</button>
          ...
          {latestWt && <p style={{ color:"rgba(255,255,255,0.85)",fontSize:12,marginBottom:12 }}>Current: <strong>{latestWt} kg</strong> · Lost: <strong>{lostWt} kg</strong> 🎉</p>}
      ...
      {view==="plan" && (<>
        ...
        <div style={{ overflowX:"auto",padding:"14px 16px 0",display:"flex",gap:8 }}>
          {DAYS.map(day=>{
            const p=getDayProg(day); const pl=DIET_PLAN[day];
            const isSel=day===selectedDay; const isToday=day===getTodayKey();
            return(
        ...
        <div style={{ margin:"0 16px 8px",background:"#FFF8F0",border:"1px solid #F0DCC8",borderRadius:12,padding:"8px 12px",display:"flex",alignItems:"center",gap:8 }}>
          ...
          <p style={{ fontSize:11,color:"#A06030" }}>Tap <strong>Swap</strong> to replace a meal with the same meal from another day · Once logged, swap is 🚫 locked</p>
        </div>
        ...
        <div style={{ padding:"4px 16px" }}>
          {dayPlan.meals.map(meal=>{
            ...
            const { text:effectiveText, isSwapped, fromDay } = getEffective(selectedDay, meal);
            return(
              <div key={meal.id} style={{ background:isLogged?"#F0F8F2":"#fff",border:isLogged?"1.5px solid #B8DEC0":isSwapped?"1.5px solid #D4A870":"1.5px solid #EDE0D4",borderRadius:14,marginBottom:10,overflow:"hidden" }}>
                {isSwapped && (
                  <div style={{ background:"linear-gradient(90deg,#FFF0D8,#FFFAF2)",padding:"5px 14px",display:"flex",alignItems:"center",gap:6,borderBottom:"1px solid #F0DCC8" }}>
                    ...
                    <span style={{ fontSize:10,fontWeight:700,color:"#C4853A",letterSpacing:0.5 }}>SWAPPED FROM {fromDay?.toUpperCase()}</span>
                ...
                <div style={{ padding:"0 14px 12px",display:"flex",justifyContent:"flex-end" }}>
                  <button onClick={e=>{ e.stopPropagation(); setModal({day:selectedDay,mealId:meal.id,label:meal.label}); }} style={{ border:isSwapped?"1.5px solid #D4A870":"1.5px solid #EDE0D4",background:isSwapped?"#FFF4E8":"#F8F2EC",borderRadius:20,padding:"5px 14px",fontSize:11,fontWeight:600,color:isLogged?"#C0A090":isSwapped?"#C4853A":"#A08070",cursor:isLogged?"not-allowed":"pointer",display:"flex",alignItems:"center",gap:5,opacity:isLogged?0.5:1,transition:"all 0.15s" }}>
                    🔄 {isSwapped?"Change Swap":"Swap"}
                  </button>
                </div>
              </div>
            );
      ...
      {view==="progress" && (
        <div style={{ padding:"16px" }}>
          ...
          {DAYS.map(day=>{
            const p=getDayProg(day); const pl=DIET_PLAN[day]; const isToday=day===getTodayKey();
          ...
          <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginTop:4 }}>
            {[
              ...
              {icon:"📉",val:lostWt==="—"?`${lostWt}`:`-${lostWt} kg`,label:"Weight Lost",color:"#5A8A6A"},
              {icon:"🎯",val:latestWt?`${(latestWt-65).toFixed(1)} kg`:"—",label:"To Goal (65kg)",color:"#D48B3A"},
          ...
          {weights.length>0 && (
            <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:10,marginBottom:16 }}>
              {[
                ...
                {label:"Lost",val:lostWt==="—"?lostWt:`${lostWt} kg`,color:"#5A8A6A"},
              ].map(s=>(
          ...
          {latestWt && (
            <div style={{ background:"#fff",borderRadius:14,padding:"14px 16px",border:"1px solid #EDE0D4",marginBottom:16 }}>
              ...
              <p style={{ fontSize:11,color:"#5A8A6A",marginTop:6,textAlign:"center",fontWeight:600 }}>
                {((((startWt-latestWt)/(startWt-65))*100)).toFixed(0)}% of journey complete 🎯
              </p>
            </div>
          )}
                  ...
                  <div key={i} style={{ background:"#fff",borderRadius:14,padding:"12px 16px",border:"1px solid #EDE0D4",marginBottom:8,display:"flex",alignItems:"center",gap:12 }}>
                    ...
                    {diff!==null && (
                      <span style={{ fontSize:13,fontWeight:700,color:parseFloat(diff)<0?"#5A8A6A":parseFloat(diff)>0?"#C4756A":"#A08070" }}>
                        {parseFloat(diff)<0?`↓ ${Math.abs(diff)} kg`:parseFloat(diff)>0?`↑ ${diff} kg`:"—"}
                      </span>
                    )}
                  </div>
                );
      ...
      <div style={{ position:"fixed",bottom:0,left:"50%",transform:"translateX(-50%)",width:"100%",maxWidth:430,background:"rgba(253,246,236,0.95)",backdropFilter:"blur(10px)",borderTop:"1px solid #EDE0D4",padding:"10px 20px",textAlign:"center" }}>
        <p style={{ fontSize:11,color:"#B0907E" }}>Tap meal to log ✓ · 🔄 Swap replaces with another day's meal</p>
      </div>
