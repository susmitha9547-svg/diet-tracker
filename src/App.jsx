...
const ALTERNATIVES = {
  ...
  breakfast: [
    ...
    { id: "a_br_8", text: "1 bowl daliya upma + boiled egg" },
  ],
  lunch: [
...
const DIET_PLAN = {
  ...
  Thursday: { color: "#5A8A6A", gradient: "linear-gradient(135deg,#5A8A6A,#6BA07A)", isVeg: true, meals: [
    ...
    { id: "midsnack",  time: "12 PM",    label: "Mid Snack", text: "4 almonds + 2 walnuts (small handful)" },
    { id: "lunch",     time: "2 PM",     label: "Lunch",     text: "2 mix veg paneer paratha + half bowl curd" },
  ...
  Saturday: { color: "#D48B3A", gradient: "linear-gradient(135deg,#D48B3A,#E09E4A)", isVeg: true, meals: [
    ...
    { id: "midsnack",  time: "12 PM",    label: "Mid Snack", text: "4 almonds + 2 walnuts (small handful)" },
    { id: "lunch",     time: "2 PM",     label: "Lunch",     text: "half bowl veg pulao/biryani + 1 bowl raita + salad" },
    { id: "snack",     time: "6 PM",     label: "Snack",     text: "1 glass green tea + 1 seasonal fruit" },
  ...
  Tuesday: { color: "#5A8A6A", gradient: "linear-gradient(135deg,#5A8A6A,#6BA07A)", isVeg: false, meals: [
    ...
    { id: "midsnack",  time: "12 PM",    label: "Mid Snack", text: "4 almonds + 2 walnuts (small handful)" },
    { id: "lunch",     time: "2 PM",     label: "Lunch",     text: "half bowl curd rice + 1 bowl dal + cucumber salad" },
    { id: "snack",     time: "6 PM",     label: "Snack",     text: "1 glass green tea + 1 seasonal fruit" },
    { id: "dinner",    time: "8 PM",     label: "Dinner",    text: "Paneer tikka (100g) + cucumber and tomato salad" },
  ]},
};
...
const getLS = (k, fb) => {
  try { const s = localStorage.getItem(k); return s ? JSON.parse(s) : fb; }
  catch { return fb; }
};
const setLS = (k, v) => {
...
export default function DietTracker() {
  ...
  useEffect(() => {
    ...
    setUsedMeals(getLS(K_USED(weekNum), []));
  ...
  const getEffective = (day, meal) => {
    ...
    if (!sw) return { text: meal.text, isSwapped: false, swapLabel: "" };
    if (sw.type === "alt") {
      const pool = ALTERNATIVES[getMealType(meal.id)] || [];
      const found = pool.find(a => a.id === sw.val);
      return found
        ? { text: found.text, isSwapped: true, swapLabel: "Alternative" }
        : { text: meal.text, isSwapped: false, swapLabel: "" };
    }
    if (sw.type === "day") {
      const parts = sw.val.split("_");
      const srcDay = parts[0];
      const srcMealId = parts[1];
      const srcMeal = DIET_PLAN[srcDay] && DIET_PLAN[srcDay].meals.find(m => m.id === srcMealId);
      return srcMeal
        ? { text: srcMeal.text, isSwapped: true, swapLabel: "Swapped with " + srcDay }
        : { text: meal.text, isSwapped: false, swapLabel: "" };
    }
    return { text: meal.text, isSwapped: false, swapLabel: "" };
  };

  const toggleMeal = (day, mealId) => {
    const wasLogged = !!(logs[day] && logs[day][mealId]);
    const updatedDay = Object.assign({}, logs[day] || {}, { [mealId]: !wasLogged });
    const newLogs = Object.assign({}, logs, { [day]: updatedDay });
    setLogs(newLogs);
    saveLog(day, updatedDay);

    const sw = swaps[day + "_" + mealId];
    if (!wasLogged) {
      // Logging: mark the eaten meal key as used
      let eatenKey = day + "_" + mealId;
      if (sw && sw.type === "day") eatenKey = sw.val;
      else if (sw && sw.type === "alt") eatenKey = sw.val;
      if (!usedMeals.includes(eatenKey)) {
        const u = usedMeals.concat([eatenKey]);
        setUsedMeals(u); saveUsed(u);
      }
      setJustChecked(day + "_" + mealId);
      setTimeout(() => setJustChecked(null), 800);
    } else {
      // Un-logging: free up the eaten meal key
      let eatenKey = day + "_" + mealId;
      if (sw && sw.type === "day") eatenKey = sw.val;
      else if (sw && sw.type === "alt") eatenKey = sw.val;
      const u = usedMeals.filter(v => v !== eatenKey);
      setUsedMeals(u); saveUsed(u);
    }
  };

  const applySwap = (day, mealId, type, val) => {
    const updated = Object.assign({}, swaps);
    if (type === "day") {
      const parts = val.split("_");
      const srcDay = parts[0];
      updated[day + "_" + mealId]    = { type: "day", val: srcDay + "_" + mealId };
      updated[srcDay + "_" + mealId] = { type: "day", val: day + "_" + mealId };
    } else {
      updated[day + "_" + mealId] = { type: type, val: val };
    }
  ...
  const resetDay = (day) => {
    const updatedLogs = Object.assign({}, logs, { [day]: {} });
    setLogs(updatedLogs); saveLog(day, {});
    const s = Object.assign({}, swaps);
    Object.keys(s).forEach(k => { if (k.indexOf(day + "_") === 0) delete s[k]; });
  ...
  const resetWeek = () => {
    const cleared = {};
    DAYS.forEach(d => { cleared[d] = {}; setLS(K_LOG(weekNum, d), {}); });
    setLogs(cleared); setSwaps({}); saveSwaps({}); setUsedMeals([]); saveUsed([]);
    setConfirmReset(null); setResetModal(false);
  };
  ...
  const addWeight = () => {
    ...
    const updated = weights.concat([entry]);
    setWeights(updated); setLS(K_WEIGHTS, updated);
    setWeightInput(""); setWeightModal(false);
  };

  const getDayProg = (day) => {
    const total = DIET_PLAN[day].meals.length;
    const done  = DIET_PLAN[day].meals.filter(m => logs[day] && logs[day][m.id]).length;
    return { done, total, pct: total ? Math.round((done / total) * 100) : 0 };
  };

  const getWeekProg = () => {
    let t = 0, d = 0;
    DAYS.forEach(day => { const p = getDayProg(day); t += p.total; d += p.done; });
  ...
  const renderSwapModal = () => {
    ...
    const isLogged   = !!(logs[day] && logs[day][mealId]);
        ...
        <div onClick={e => e.stopPropagation()} style={{ background: "#FDF6EC", borderRadius: "20px 20px 0 0", width: "100%", maxWidth: 430, maxHeight: "82vh", display: "flex", flexDirection: "column", boxShadow: "0 -8px 40px rgba(44,24,16,0.2)" }}>
          <div style={{ padding: "14px 20px 0", flexShrink: 0 }}>
            ...
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 12 }}>
              <div>
                <p style={{ fontSize: 11, color: "#A08070", textTransform: "uppercase", letterSpacing: 1, marginBottom: 3 }}>Replace - {selectedDay} {label}</p>
                <h3 style={{ fontFamily: "'Playfair Display',serif", fontSize: 18, color: "#2C1810" }}>Choose replacement</h3>
              </div>
              <button onClick={() => setModal(null)} style={{ border: "none", background: "#F0E4DA", borderRadius: "50%", width: 32, height: 32, fontSize: 16, cursor: "pointer", color: "#A08070" }}>X</button>
            </div>
            {isLogged && (
              <div style={{ background: "#FFF0EE", border: "1px solid #F5C4BC", borderRadius: 10, padding: "8px 12px", marginBottom: 10 }}>
                <p style={{ fontSize: 11, color: "#C4756A", fontWeight: 600 }}>Un-log this meal first to change the swap.</p>
              </div>
            )}
            <div style={{ display: "flex", background: "#F0E4DA", borderRadius: 12, padding: 3, marginBottom: 0 }}>
              {[["day", "Swap from Day"], ["alt", "Alternatives"]].map(([t, l]) => (
                <button key={t} onClick={() => setActiveTab(t)} style={{ flex: 1, padding: "8px", border: "none", borderRadius: 10, background: activeTab === t ? "#fff" : "transparent", fontSize: 12, fontWeight: 700, color: activeTab === t ? "#C4756A" : "#A08070", cursor: "pointer" }}>{l}</button>
              ))}
            </div>
          </div>
          ...
          <div style={{ overflowY: "auto", padding: "12px 16px 28px", flex: 1 }}>
            {currentSw && !isLogged && (
              <div onClick={() => removeSwap(day, mealId)} style={{ border: "1.5px solid #EDE0D4", borderRadius: 14, padding: "12px 14px", marginBottom: 10, cursor: "pointer", background: "#fff", display: "flex", alignItems: "center", gap: 10 }}>
                <div style={{ width: 32, height: 32, borderRadius: "50%", background: "#F0E4DA", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 16, flexShrink: 0 }}>R</div>
                <div>
            ...
            {activeTab === "day" && (
              <div>
                {otherDays.length === 0 ? (
                  <p style={{ textAlign: "center", padding: "24px 0", color: "#A08070", fontSize: 13 }}>No other days have a {label} to swap with.</p>
            ...
            {activeTab === "alt" && (
              <div>
                {altPool.length === 0 ? (
                  <p style={{ textAlign: "center", padding: "24px 0", color: "#A08070", fontSize: 13 }}>No alternatives for this meal type.</p>
                ) : altPool.map(option => {
                  const isCurrent = currentSw && currentSw.type === "alt" && currentSw.val === option.id;
                  const isEaten   = usedMeals.includes(option.id) && !isCurrent;
                  return (
                    <div key={option.id} onClick={() => { if (!isEaten && !isLogged) applySwap(day, mealId, "alt", option.id); }} style={{ border: isCurrent ? "2px solid #5A8A6A" : isEaten ? "1.5px dashed #D4C4B8" : "1.5px solid #EDE0D4", background: isCurrent ? "#F0F8F2" : isEaten ? "#F8F4F0" : "#fff", borderRadius: 14, padding: "12px 14px", marginBottom: 10, cursor: isEaten || isLogged ? "not-allowed" : "pointer", opacity: isEaten ? 0.5 : 1, display: "flex", alignItems: "flex-start", gap: 10 }}>
                      <div style={{ width: 30, height: 30, borderRadius: "50%", flexShrink: 0, marginTop: 1, background: isCurrent ? "#5A8A6A" : isEaten ? "#D4C4B8" : "#F0E4DA", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 13, color: isCurrent ? "#fff" : "#A08070" }}>
                        {isCurrent ? "V" : isEaten ? "X" : ">"}
                      </div>
                      <div style={{ flex: 1 }}>
                        <p style={{ fontSize: 13, color: isEaten ? "#B0907E" : "#2C1810", lineHeight: 1.5 }}>{option.text}</p>
                        {isCurrent && <p style={{ fontSize: 10, color: "#5A8A6A", fontWeight: 700, marginTop: 3 }}>Currently selected</p>}
                        {isEaten   && <p style={{ fontSize: 10, color: "#B0907E", marginTop: 3 }}>Already eaten this week</p>}
                      </div>
                    </div>
                  );
                })}
    ...
    return (
      <div onClick={() => { setResetModal(false); setConfirmReset(null); }} style={{ position: "fixed", inset: 0, background: "rgba(44,24,16,0.6)", zIndex: 999, display: "flex", alignItems: "flex-end", justifyContent: "center" }}>
        <div onClick={e => e.stopPropagation()} style={{ background: "#FDF6EC", borderRadius: "20px 20px 0 0", width: "100%", maxWidth: 430, padding: "20px 20px 36px", boxShadow: "0 -8px 40px rgba(44,24,16,0.2)" }}>
          ...
          <h3 style={{ fontFamily: "'Playfair Display',serif", fontSize: 20, color: "#2C1810", marginBottom: 4 }}>Reset and Week Options</h3>
          <p style={{ fontSize: 12, color: "#A08070", marginBottom: 16 }}>Week {weekNum} (Wed to Tue)</p>
          {confirmReset ? (
            ...
            <div>
              {[
                { icon: "D", label: "Reset Today (" + selectedDay + ")", sub: "Clear logs and swaps for this day only", color: "#C4756A", action: "day" },
                { icon: "W", label: "Reset Entire Week",                  sub: "Clear all logs and swaps for week " + weekNum, color: "#D48B3A", action: "week" },
                { icon: "N", label: "Start New Week",                     sub: "Week " + (weekNum + 1) + " - Fresh start, old data saved", color: "#5A8A6A", action: "newweek" },
              ].map(item => (
                <div key={item.action} onClick={() => setConfirmReset(item.action)} style={{ display: "flex", alignItems: "center", gap: 12, background: "#fff", border: "1.5px solid #EDE0D4", borderRadius: 14, padding: "14px", marginBottom: 10, cursor: "pointer" }}>
                  <div style={{ width: 42, height: 42, borderRadius: 12, background: item.color + "22", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 18, fontWeight: 700, color: item.color, flexShrink: 0 }}>{item.icon}</div>
                  <div>
    ...
    return (
      <div onClick={() => setWeightModal(false)} style={{ position: "fixed", inset: 0, background: "rgba(44,24,16,0.6)", zIndex: 999, display: "flex", alignItems: "flex-end", justifyContent: "center" }}>
        <div onClick={e => e.stopPropagation()} style={{ background: "#FDF6EC", borderRadius: "20px 20px 0 0", width: "100%", maxWidth: 430, padding: "20px 20px 40px", boxShadow: "0 -8px 40px rgba(44,24,16,0.2)" }}>
          <div style={{ width: 36, height: 4, background: "#D4C4B8", borderRadius: 99, margin: "0 auto 16px" }} />
          <h3 style={{ fontFamily: "'Playfair Display',serif", fontSize: 20, color: "#2C1810", marginBottom: 4 }}>Log Weight</h3>
          <p style={{ fontSize: 12, color: "#A08070", marginBottom: 20 }}>{new Date().toLocaleDateString("en-IN")} - Week {weekNum}</p>
          <div style={{ background: "#fff", border: "2px solid #C4756A", borderRadius: 14, padding: "14px 16px", display: "flex", alignItems: "center", gap: 8, marginBottom: 16 }}>
  ...
  return (
    <div style={{ fontFamily: "'DM Sans',sans-serif", background: "#FDF6EC", minHeight: "100vh", maxWidth: 430, margin: "0 auto", paddingBottom: 80 }}>
      ...
      <div style={{ background: "linear-gradient(135deg,#9E4F45 0%,#C4756A 60%,#D48B3A 100%)", padding: "36px 20px 24px", position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ position: "relative" }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 4 }}>
            <p style={{ color: "rgba(255,255,255,0.75)", fontSize: 11, letterSpacing: 2, textTransform: "uppercase" }}>Week {weekNum} - Wed to Tue</p>
            <div style={{ display: "flex", gap: 8 }}>
              <button onClick={() => setWeightModal(true)} style={{ border: "none", background: "rgba(255,255,255,0.2)", borderRadius: 20, padding: "5px 12px", fontSize: 11, fontWeight: 600, color: "#fff", cursor: "pointer" }}>Weight</button>
              <button onClick={() => setResetModal(true)}  style={{ border: "none", background: "rgba(255,255,255,0.2)", borderRadius: 20, padding: "5px 12px", fontSize: 11, fontWeight: 600, color: "#fff", cursor: "pointer" }}>Reset</button>
            </div>
          </div>
          <h1 style={{ fontFamily: "'Playfair Display',serif", color: "#fff", fontSize: 26, marginBottom: 4 }}>Diet Tracker</h1>
          {latestWt && <p style={{ color: "rgba(255,255,255,0.85)", fontSize: 12, marginBottom: 12 }}>Current: <strong>{latestWt} kg</strong>{lostWt ? " - Lost: " : ""}<strong>{lostWt ? lostWt + " kg" : ""}</strong></p>}
      ...
      <div style={{ display: "flex", background: "#fff", borderBottom: "1px solid #EDE0D4", padding: "4px 16px" }}>
        {[["plan", "Plan"], ["progress", "Progress"], ["weight", "Weight"]].map(([tab, lbl]) => (
          <button key={tab} onClick={() => setView(tab)} style={{ flex: 1, padding: "10px 0", border: "none", background: "none", fontFamily: "'DM Sans',sans-serif", fontSize: 12, fontWeight: 600, color: view === tab ? "#C4756A" : "#A08070", borderBottom: view === tab ? "2px solid #C4756A" : "2px solid transparent", cursor: "pointer" }}>{lbl}</button>
        ))}
      </div>
      ...
      {view === "plan" && (
        <div>
          <div style={{ overflowX: "auto", padding: "14px 16px 0", display: "flex", gap: 8 }}>
            {DAYS.map(day => {
              const p = getDayProg(day);
              const pl = DIET_PLAN[day];
              ...
              return (
                <button key={day} className="db" onClick={() => setSelectedDay(day)} style={{ flexShrink: 0, padding: "8px 12px", borderRadius: 12, minWidth: 60, border: isSel ? "none" : "1.5px solid #EDE0D4", background: isSel ? pl.gradient : "#fff", cursor: "pointer", position: "relative" }}>
                  ...
                  {pl.isVeg && <div style={{ fontSize: 8, marginTop: 1, color: isSel ? "#fff" : "#5A8A6A" }}>VEG</div>}
                </button>
              );
            })}
          </div>
          ...
          <div style={{ margin: "0 16px 8px", background: "#FFF8F0", border: "1px solid #F0DCC8", borderRadius: 12, padding: "8px 12px", display: "flex", alignItems: "center", gap: 8 }}>
            <p style={{ fontSize: 11, color: "#A06030" }}>Tap <strong>Day Swap</strong> to exchange with another day - <strong>Alternatives</strong> for different options</p>
          </div>

          <div style={{ padding: "4px 16px" }}>
            {dayPlan.meals.map(meal => {
              const isLogged = !!(logs[selectedDay] && logs[selectedDay][meal.id]);
              const isJust   = justChecked === selectedDay + "_" + meal.id;
              const effective = getEffective(selectedDay, meal);
              return (
                <div key={meal.id} style={{ background: isLogged ? "#F0F8F2" : "#fff", border: isLogged ? "1.5px solid #B8DEC0" : effective.isSwapped ? "1.5px solid #D4A870" : "1.5px solid #EDE0D4", borderRadius: 14, marginBottom: 10, overflow: "hidden" }}>
                  {effective.isSwapped && (
                    <div style={{ background: "linear-gradient(90deg,#FFF0D8,#FFFAF2)", padding: "5px 14px", display: "flex", alignItems: "center", gap: 6, borderBottom: "1px solid #F0DCC8" }}>
                      <span style={{ fontSize: 10, fontWeight: 700, color: "#C4853A", letterSpacing: 0.5 }}>{effective.swapLabel.toUpperCase()}</span>
                  ...
                  <div className="mc" onClick={() => toggleMeal(selectedDay, meal.id)} style={{ display: "flex", alignItems: "flex-start", gap: 12, padding: "14px 14px 10px", cursor: "pointer" }}>
                    <div className={isJust ? "cb" : ""} style={{ width: 28, height: 28, borderRadius: "50%", flexShrink: 0, background: isLogged ? "linear-gradient(135deg,#5A8A6A,#6BA07A)" : "transparent", border: isLogged ? "none" : "2px solid #D4C4B8", display: "flex", alignItems: "center", justifyContent: "center", marginTop: 1 }}>
                      {isLogged && <span style={{ color: "#fff", fontSize: 14 }}>V</span>}
                    </div>
                    <div style={{ flex: 1 }}>
                      ...
                      <p style={{ fontSize: 13, color: isLogged ? "#6B9B7A" : "#3A2218", lineHeight: 1.5, opacity: isLogged ? 0.8 : 1 }}>{effective.text}</p>
                  ...
                  <div style={{ padding: "0 14px 12px", display: "flex", justifyContent: "flex-end", gap: 8 }}>
                    <button onClick={e => { e.stopPropagation(); setActiveTab("day"); setModal({ day: selectedDay, mealId: meal.id, label: meal.label }); }} style={{ border: "1.5px solid #EDE0D4", background: "#F8F2EC", borderRadius: 20, padding: "5px 12px", fontSize: 11, fontWeight: 600, color: isLogged ? "#C0A090" : "#A08070", cursor: isLogged ? "not-allowed" : "pointer", opacity: isLogged ? 0.5 : 1 }}>
                      Day Swap
                    </button>
                    <button onClick={e => { e.stopPropagation(); setActiveTab("alt"); setModal({ day: selectedDay, mealId: meal.id, label: meal.label }); }} style={{ border: effective.isSwapped && swaps[selectedDay + "_" + meal.id] && swaps[selectedDay + "_" + meal.id].type === "alt" ? "1.5px solid #D4A870" : "1.5px solid #EDE0D4", background: effective.isSwapped && swaps[selectedDay + "_" + meal.id] && swaps[selectedDay + "_" + meal.id].type === "alt" ? "#FFF4E8" : "#F8F2EC", borderRadius: 20, padding: "5px 12px", fontSize: 11, fontWeight: 600, color: isLogged ? "#C0A090" : effective.isSwapped && swaps[selectedDay + "_" + meal.id] && swaps[selectedDay + "_" + meal.id].type === "alt" ? "#C4853A" : "#A08070", cursor: isLogged ? "not-allowed" : "pointer", opacity: isLogged ? 0.5 : 1 }}>
                      Alternatives
                    </button>
                  </div>
                </div>
      ...
      {view === "progress" && (
        <div style={{ padding: "16px" }}>
          ...
          <div style={{ background: "#fff", borderRadius: 16, padding: 20, border: "1px solid #EDE0D4", marginBottom: 14, textAlign: "center" }}>
            <div style={{ position: "relative", width: 110, height: 110, margin: "0 auto 12px" }}>
              <svg width="110" height="110" style={{ transform: "rotate(-90deg)" }}>
                ...
                <circle cx="55" cy="55" r="46" fill="none" stroke="url(#grad)" strokeWidth="10" strokeDasharray={String(2 * Math.PI * 46)} strokeDashoffset={String(2 * Math.PI * 46 * (1 - weekProg.pct / 100))} strokeLinecap="round" style={{ transition: "stroke-dashoffset 0.8s ease" }} />
                <defs>
                  <linearGradient id="grad" x1="0%" y1="0%" x2="100%" y2="0%">
                    <stop offset="0%" stopColor="#C4756A" />
                    <stop offset="100%" stopColor="#D48B3A" />
                  </linearGradient>
                </defs>
              </svg>
          ...
          {DAYS.map(day => {
            ...
            const pl = DIET_PLAN[day];
            const isToday = day === getTodayKey();
            const sc = DIET_PLAN[day].meals.filter(m => swaps[day + "_" + m.id]).length;
            return (
              <div key={day} onClick={() => { setSelectedDay(day); setView("plan"); }} style={{ background: "#fff", borderRadius: 14, padding: "12px 16px", border: isToday ? "1.5px solid " + pl.color : "1.5px solid #EDE0D4", marginBottom: 10, cursor: "pointer", display: "flex", alignItems: "center", gap: 14 }}>
                ...
                <div style={{ flex: 1 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                    ...
                    {pl.isVeg  && <span style={{ background: "#E8F2EC", color: "#5A8A6A", fontSize: 9, fontWeight: 700, padding: "1px 6px", borderRadius: 20 }}>VEG</span>}
                    {isToday   && <span style={{ background: "#FFE8E5", color: "#C4756A", fontSize: 9, fontWeight: 700, padding: "1px 6px", borderRadius: 20 }}>TODAY</span>}
                    {sc > 0    && <span style={{ background: "#FFF0D8", color: "#C4853A", fontSize: 9, fontWeight: 700, padding: "1px 6px", borderRadius: 20 }}>{sc} swap{sc > 1 ? "s" : ""}</span>}
                  </div>
      ...
      {view === "weight" && (
        <div style={{ padding: "16px" }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16 }}>
            <div>
              <h2 style={{ fontFamily: "'Playfair Display',serif", fontSize: 20, color: "#2C1810" }}>Weight Log</h2>
          ...
          {weights.length > 0 && (
            <div>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 10, marginBottom: 16 }}>
                {[
                  { label: "Start",   val: weights[0].weight + " kg", color: "#A08070" },
                  { label: "Current", val: latestWt ? latestWt + " kg" : "--", color: "#C4756A" },
                  { label: "Lost",    val: lostWt ? lostWt + " kg" : "--", color: "#5A8A6A" },
                ].map(s => (
              ...
              <div style={{ background: "#fff", borderRadius: 14, padding: "14px 16px", border: "1px solid #EDE0D4", marginBottom: 16 }}>
                <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 8 }}>
                  ...
                  <span style={{ fontSize: 12, color: "#A08070" }}>{latestWt ? Math.max(0, latestWt - 65).toFixed(1) : "--"} kg to go</span>
                </div>
                <div style={{ background: "#F0E4DA", borderRadius: 99, height: 10, overflow: "hidden" }}>
                  <div style={{ height: "100%", borderRadius: 99, background: "linear-gradient(90deg,#C4756A,#5A8A6A)", width: Math.min(100, Math.max(0, ((startWt - (latestWt || startWt)) / (startWt - 65)) * 100)).toFixed(0) + "%", transition: "width 0.5s ease" }} />
                </div>
                <p style={{ fontSize: 11, color: "#5A8A6A", marginTop: 6, textAlign: "center", fontWeight: 600 }}>
                  {Math.min(100, Math.max(0, ((startWt - (latestWt || startWt)) / (startWt - 65)) * 100)).toFixed(0)}% of journey complete
                </p>
              </div>
            </div>
          )}
          {weights.length === 0 ? (
            ...
            <div>
              ...
              {weights.slice().reverse().map((entry, i) => {
                ...
                return (
                  <div key={i} style={{ background: "#fff", borderRadius: 14, padding: "12px 16px", border: "1px solid #EDE0D4", marginBottom: 8, display: "flex", alignItems: "center", gap: 12 }}>
                    ...
                    {diff !== null && (
                      <span style={{ fontSize: 13, fontWeight: 700, color: parseFloat(diff) < 0 ? "#5A8A6A" : parseFloat(diff) > 0 ? "#C4756A" : "#A08070" }}>
                        {parseFloat(diff) < 0 ? "down " + Math.abs(diff) + " kg" : parseFloat(diff) > 0 ? "up " + diff + " kg" : "--"}
                      </span>
                    )}
                  </div>
                );
      ...
      <div style={{ position: "fixed", bottom: 0, left: "50%", transform: "translateX(-50%)", width: "100%", maxWidth: 430, background: "rgba(253,246,236,0.95)", backdropFilter: "blur(10px)", borderTop: "1px solid #EDE0D4", padding: "10px 20px", textAlign: "center" }}>
        <p style={{ fontSize: 11, color: "#B0907E" }}>Tap meal to log - Day Swap exchanges meals - Alternatives replaces meal</p>
      </div>
